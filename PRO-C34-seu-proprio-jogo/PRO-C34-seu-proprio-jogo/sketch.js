...
function preload() {
  ...
  cannonBoom = loadSound("assets/cannon_explosion.mp3");
  cannonWater = loadSound("assets/cannon_water.mp3");
  evilLaugh = loadSound("assets/pirate_laugh.mp3")
}
...
function draw() {
  ...
  if (!backgroundSound.isPlaying()){
    ...
    backgroundSound.setVolume(100000000)
  }
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      if (collision.collided && !boats[i].isBroken) {
        isGameOver = true;
        gameOver();
        if (!isLaughing && !evilLaugh.isPlaying()){
          evilLaugh.play()
          isLaughing = true
        }
      }
